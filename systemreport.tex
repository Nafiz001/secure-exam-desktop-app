...
\begin{figure}[H]
  ...
  \fbox{\parbox{5in}{\centering
    \vspace{4pt}
    \texttt{[Student Machine]} \\
    Electron Main Process (lockdown, violation pipeline, process scanner) \\
    $\updownarrow$ preload bridge $\updownarrow$ \\
    React Renderer (Student/Teacher/Admin dashboards, BlazeFace proctoring) \\
    $\Updownarrow$ HTTPS \\
    \texttt{[Department Server]} \\
    Express Backend (Auth, Exams, Submissions, Proctoring, Admin, AI, CodeRunner) \\
    $\Updownarrow$ \\
    PostgreSQL Database (7 core tables, additive migrations)
    \vspace{4pt}
  }}
  \caption{High level architecture of the Invigilo system.}
  \label{fig:3.1}
\end{figure}
...
\begin{figure}[H]
  ...
  \fbox{\parbox{5in}{\centering
    \vspace{4pt}
    Login or Room Code Entry \\
    $\downarrow$ \\
    Waiting Room (auto polling at 3s) \\
    $\downarrow$ (teacher starts exam) \\
    Exam Mode Lock (fullscreen, always on top) \\
    $\downarrow$ \\
    Question Attempt (MCQ / Written / Coding) \\
    $\downarrow$ (submit, timeout or force submit) \\
    Submission Persisted, Lock Released \\
    $\downarrow$ (after evaluation by teacher) \\
    Result View with Per Question Feedback
    \vspace{4pt}
  }}
  \caption{Student attempt workflow inside Invigilo.}
  \label{fig:3.2}
\end{figure}
